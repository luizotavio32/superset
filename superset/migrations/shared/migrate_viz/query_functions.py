...
EXTRA_FORM_DATA_OVERRIDE_EXTRA_KEYS = ["relative_start", "relative_end", "time_grain_sqla"]
...
EXTRA_FORM_DATA_OVERRIDE_REGULAR_KEYS = list(EXTRA_FORM_DATA_OVERRIDE_REGULAR_MAPPINGS.keys())
...
def is_time_comparison(form_data: dict, query_object: dict) -> bool:
    ...
    return comparison_type in [ct.value for ct in ComparisonType]  and len(metric_offset_map) > 0

def ensure_is_int(value, default_value=None) -> int:
    """
    Convert the given value to an integer.
    If conversion fails, returns default_value if provided, otherwise returns NaN (as float('nan')).
    """
    try:
        val = int(str(value))
...
def is_physical_column(column=None):
    return isinstance(column, str)

def is_adhoc_column(column=None):
    return (
        not isinstance(column, str) and
        hasattr(column, 'sqlExpression') and column.sqlExpression is not None and
        hasattr(column, 'label') and column.label is not None and
        (not hasattr(column, 'expressionType') or column.expressionType == 'SQL')
    )

def is_query_form_column(column):
    return is_physical_column(column) or is_adhoc_column(column)

def is_x_axis_set(form_data):
    return is_query_form_column(form_data.get("x_axis"))

def get_x_axis_column(form_data):
    # The form_data should be "raw form_data" -- the snake_case version of form_data rather than camelCase.
...
def get_x_axis_label(form_data):
...
def time_compare_pivot_operator(form_data, query_object):
    ...
    columns = query_object.get("series_columns") or query_object.get("columns")
    ...
    if x_axis_label and metric_labels:
        return {
            ...
            'options': {
                ...
                'aggregates': {metric: {'operator': 'mean'} for metric in metric_labels},
...
def normalize_order_by(query_object):
    if isinstance(query_object.get('orderby'), list) and len(query_object.get('orderby', [])) > 0:
        # ensure a valid orderby clause
        orderby_clause = query_object['orderby'][0]
        if (isinstance(orderby_clause, list) and 
            len(orderby_clause) == 2 and 
            orderby_clause[0] and 
            isinstance(orderby_clause[1], bool)):
            return query_object
    ...
    if query_object.get('series_limit_metric') is not None and query_object.get('series_limit_metric'):
        return {
            **clone_query_object,
    ...
    if query_object.get('legacy_order_by') is not None and query_object.get('legacy_order_by'):
        return {
            **clone_query_object,
            'orderby': [[query_object['legacy_order_by'], is_asc]]
        }
    
    if isinstance(query_object.get('metrics'), list) and len(query_object.get('metrics', [])) > 0:
        return {
            **clone_query_object,
            'orderby': [[query_object['metrics'][0], is_asc]]
        }
...
def extract_query_fields(form_data, aliases=None):
    ...
    # Update with any provided aliases
    if aliases:
        query_field_aliases.update(aliases)
    
    # Extract query_mode and the rest of form_data
    ...
    for key, value in rest_form_data.items():
        ...
        # ignore groupby and metrics in raw records mode
    ...
    result = {
        "columns": remove_duplicates([col for col in columns if col != ""], get_column_label),
        "orderby": None,
    }
    
    # Only include metrics if not in raw mode
    if query_mode != "raw":
        result["metrics"] = remove_duplicates(metrics, get_metric_label)
    else:
        result["metrics"] = None
    
    # Process orderby if it exists
    if orderby:
        result["orderby"] = []
        for item in orderby:
            ...
            if isinstance(item, str):
                try:
                    ...
                    raise ValueError("Found invalid orderby options")
            else:
...
def extract_extras(form_data: dict) -> dict:
    ...
    extras["time_grain_sqla"] = extract.get("time_grain_sqla", form_data.get("time_grain_sqla"))
    extract["granularity"] = (
...
def override_extra_form_data(query_object: dict, override_form_data: dict) -> dict:
    ...
    Mimics the behavior of the TypeScript function:
      - For keys in EXTRA_FORM_DATA_OVERRIDE_REGULAR_MAPPINGS, if set in override_form_data,
        assign the value in query_object under the mapped target key.
      - For keys in EXTRA_FORM_DATA_OVERRIDE_EXTRA_KEYS, if present in override_form_data,
        add them to query_object['extras'].
    """
...
def build_query_object(form_data, query_fields=None):
    ...
    residual_form_data = {k: v for k, v in form_data.items() if k not in [
        'annotation_layers', 'extra_form_data', 'time_range', 'since', 'until',
        'row_limit', 'row_offset', 'order_desc', 'limit', 'timeseries_limit_metric',
        'granularity', 'url_params', 'custom_params', 'series_columns',
        'series_limit', 'series_limit_metric'
    ]}
    
    # Extract fields from extra_form_data
    append_adhoc_filters = extra_form_data.get('adhoc_filters', []) if extra_form_data else []
    append_filters = extra_form_data.get('filters', []) if extra_form_data else []
    custom_form_data = extra_form_data.get('custom_form_data', {}) if extra_form_data else {}
    overrides = {k: v for k, v in extra_form_data.items() if k not in ['adhoc_filters', 'filters', 'custom_form_data']} if extra_form_data else {}
...
def normalize_time_column(form_data: dict, query_object: dict) -> dict:
    """
    If x_axis is set in form_data, find its index in query_object's columns and update that column
    with timeGrain and columnType information. The updated query_object omits the 'is_timeseries' key.
    """
    ...
    for idx, col in enumerate(_columns):
        if (is_physical_column(col) and is_physical_column(x_axis) and col == x_axis) or \
           (is_adhoc_column(col) and is_adhoc_column(x_axis) and col.get("sqlExpression") == x_axis.get("sqlExpression")):
            axis_idx = idx
            break
...
def build_query_context(
    ...
    if callable(options):
        ...
        query_fields = {}
        build_query = lambda x: [x]
    
    queries = build_query(build_query_object(form_data, query_fields))
    
    # --- query mutator begin ---
    # todo: move the query mutator into build_query_object instead of build_query_context
    for query in queries:
        if isinstance(query.get('post_processing'), list):
            # Filter out None values from post_processing
...
def rolling_window_operator(form_data: dict, query_object: dict):
    """
    Builds a post-processing configuration for a rolling window or cumulative computation.
...
def time_compare_operator(form_data: Dict[str, Any], query_object: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    """
    ...
    if is_time_comparison(form_data, query_object) and comparison_type != ComparisonType.Values.value:
        return {
            "operation": "compare",
            "options": {
...
def resample_operator(form_data: dict, query_object: dict) -> dict:
    ...
    resample_method = "asfreq" if resample_zero_fill else form_data.get("resample_method")
...
def rename_operator(form_data: Dict[str, Any], query_object: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    """
    ...
    """
    metrics = ensure_is_array(query_object.get("metrics"))
    columns = ensure_is_array(query_object.get("series_columns") or query_object.get("columns"))
    ...
    if (
        ...
        x_axis_label and
        not (
            is_time_comparison(form_data, query_object) and 
            form_data.get("comparison_type") in {ComparisonType.Difference, ComparisonType.Ratio, ComparisonType.Percentage}
        ) and
        truncate_metric is not None and bool(truncate_metric)
    ):
        rename_pairs: List[Tuple[str, Optional[str]]] = []

        if is_time_comparison(form_data, query_object) and form_data.get("comparison_type") == ComparisonType.Values:
            metric_offset_map = get_metric_offsets_map(form_data, query_object)
            time_offsets = ensure_is_array(form_data.get("time_compare"))
            for metric_with_offset in list(metric_offset_map.keys()):
                # Find the first time offset that is included in the metric_with_offset string.
                offset_label = next((offset for offset in time_offsets if offset in metric_with_offset), None)
                rename_pairs.append((metric_with_offset, offset_label))
        
        # Append the main metric label with a null (None) new name.
...
def sort_operator(form_data: Dict[str, Any], query_object: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    """
    ...
    sortable_labels += [get_metric_label(m) for m in ensure_is_array(form_data.get("metrics"))]
...
def prophet_operator(form_data: dict, query_object: dict) -> dict:
    """
    Returns a post-processing configuration for prophet forecasting if forecast is enabled and an x-axis label is present.
    """
    x_axis_label = get_x_axis_label(form_data)
